...
ApplicationWindow  {
    ...
    property var help_text_array: [
        "<b>Часть 1. Общая информация.</b><br>   Данное программное средство представляет из себя симулятор распространения звука в замкнутом помещении.
            Вам будут выданы 3 задания для выполнения. Интерфейс программы состоит из кнопки помощи (слева вверху), описания текущего задания (вверху), общего прогресса выполнения (справа ввреху),
            ...
            Консоль предназначена для просмотра состояния программы. Все большие поля в программе прокручиваются.
            Если программа будет перезапущена, вы будете вынуждены начинать работу сначала. Для проверки ваших результатов, в тестовой программе нажмите 'проверить выполнение'",
        "<b>Часть 2. Управление.</b><br>   Основную информацию вы можете видеть на анимированных картинках. <i>Каждый блок обладает уникальными свойствами</i>. Стеклянная панель отлично пропускает
            ...
            Ваша задача, комбинируя блоки, добиться выполнения всех задач в списке справа (должны загореться зеленым). Источник звука на карте обозначен оранжевым кругом с динамиком внутри.
            Рабочее место обозначено коричневым кругом с надписью РМ.",
        "<b>Часть 3. Выполнение.</b><br>  Чтобы приступить к работе, нажмите кнопку 'Перейти к выполнению'. Располагая блоки, добейтесь выполнения всех условий. Для просмотра измерений, сделанных с помощью прибора Экофизика 110А, выберете пункт 'просмотр шумового загрязнения'.
            Он отображается на карте с помощью цветовой палитры и значений в дБ. Обратите внимание, что не все клетки доступны для модификации. Для просмотра доступных клеток, выберите 'просмотр заблокированных клеток'.
            Комбинируйте блоки для достижения найлучшего эффекта, постарайтесь отследить, как распростаняется звук.
            Если Вам нужна дополнительная помощь, в папке с программой есть соотв. файлы. После окончания выполнения, кнопки управления отключатся, и вам будет предложено перейти к основной программе.
            До нажатия кнопки 'проверить выполнение' не завершайте работу программы. Если основная программа не распознаёт результаты, воспользуйтесь дополнительной утилитой в папке программы.
